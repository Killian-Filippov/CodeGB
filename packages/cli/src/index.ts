...
import {
  ...
  renderStartupError,
  runStartupChecks,
} from '../../core/src/startup/preflight';
...
export const runCli = async (argv: string[]): Promise<CliResult> => {
  ...
  try {
    ...
    if (command === 'init') {
      ...
      if (!repoPath) {
        return {
          exitCode: 1,
          stdout: '',
          stderr: renderStartupError(new StartupError('E_USAGE', 'init requires <repoPath>')),
        };
      }
    ...
    if (command === 'query') {
      ...
      if (!query) {
        return {
          exitCode: 1,
          stdout: '',
          stderr: renderStartupError(new StartupError('E_USAGE', 'query requires <query>')),
        };
      }
      const storagePath = resolveStoragePath(rest);
      await runCliStartupChecks(storagePath);
    ...
    return {
      exitCode: 1,
      stdout: '',
      stderr: renderStartupError(new StartupError('E_USAGE', `Unknown command: ${command}`)),
    };
  } catch (error) {
    return {
      exitCode: 1,
      stdout: '',
      stderr: renderStartupError(error),
    };
  }
};
