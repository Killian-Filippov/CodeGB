...
const FIELD_REGEX = /^\s*((?:(?:public|protected|private|static|final|volatile|transient)\s+)*)((?:[A-Za-z_][\w<>\[\].?]*))\s+([A-Za-z_]\w*)\s*(?:=[^;]+)?;\s*$/;
...
const extractFields = (classBody, classStartLine) => {
    const fields = [];
...
const extractMethods = (className, classBody, absoluteStart, fullSource) => {
    const methods = [];
...
export const parseJavaSource = (source, filePath) => {
    ...
    while (typeMatch) {
        ...
        const kind = kindToken === 'interface'
            ? 'Interface'
            : kindToken === 'enum'
                ? 'Enum'
                : kindToken === '@interface'
