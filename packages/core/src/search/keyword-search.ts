import type { JavaGraphNode, JavaSearchResult, KnowledgeGraph } from '../types/graph';
...
const toDocumentText = (node: JavaGraphNode): string => {
  return [
...
const buildIndex = (graph: KnowledgeGraph): { docs: IndexEntry[]; idf: Map<string, number>; avgLength: number } => {
  const docs: IndexEntry[] = graph.nodes
...
export const searchByKeyword = (graph: KnowledgeGraph, query: string, limit = 10): JavaSearchResult[] => {
  ...
  const scored: JavaSearchResult[] = docs
    .map((doc) => {
      let score = 0;
      for (const token of tokens) {
