...
import { JAVA_SCHEMA_QUERIES } from './schema.ts';
...
const selectReturnColumns = (row, returnClause) => {
...
export const executeCypherInMemory = (graph, query) => {
    const limit = extractLimit(query);
    const relMatch = query.match(/MATCH\s*\((\w+):(\w+)\)\s*-\[\s*(\w+)?(?:\s*:\s*\w+)?(?:\s*\{\s*type\s*:\s*'([A-Z_]+)'\s*\})?\s*\]\s*->\s*\((\w+):(\w+)\)\s*RETURN\s+([\s\S]+)/i);
    if (relMatch) {
